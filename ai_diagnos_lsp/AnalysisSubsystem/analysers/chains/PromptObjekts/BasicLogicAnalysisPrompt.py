...
from .prompts.general_logic_analysis_system_prompt import general_logic_analysis_system_prompt_with_overrides

if TYPE_CHECKING:
    from ai_diagnos_lsp.default_config import user_config

def BasicLogicAnalysisPromptFactory(config: user_config, filetype: str) -> ChatPromptTemplate:
    """
    ...
    result = ChatPromptTemplate.from_messages([
        ("system", f"{general_logic_analysis_system_prompt_with_overrides(ovrd)}"),
        ("human", "\n{{{file_content}}}\n\n"),
